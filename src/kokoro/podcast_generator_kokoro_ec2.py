...
EC2 Version - Optimized for IAM role authentication
"""
...
try:
    ...
    print("Error: Kokoro module not found.")
    print("Install with: pip install kokoro>=0.9.2 soundfile")
    print("Also ensure espeak-ng is installed: sudo apt-get install espeak-ng")
    exit(1)
...
class PodcastGenerator:
    def __init__(self, use_gpu=True, aws_region='us-west-2', seed=None, speech_speed=0.85, 
        ...
        """Initialize the podcast generator with Kokoro-82M - EC2 optimized"""
        ...
        # Initialize Bedrock client - simplified for EC2 with IAM roles
        print("Initializing AWS Bedrock client...")
        try:
            # On EC2 with IAM roles, this should just work
            ...
            print("✓ AWS Bedrock client initialized (using IAM role)")
        except Exception as e:
            print(f"Error initializing Bedrock: {e}")
            print("Ensure your EC2 instance has an IAM role with bedrock:InvokeModel permission")
            raise
        ...
        try:
            ...
            print(f"✓ Kokoro-82M model loaded successfully on {self.device}")
...
Remember: This is a conversation between two people discussing interesting content. Keep it natural, well-paced, and engaging."""
        ...
        try:
            ...
            for i, (speaker, text) in enumerate(segments):
                ...
                try:
                    ...
                    for j, (gs, ps, audio_chunk) in enumerate(generator):
    ...
    def create_podcast(self, content_source, output_file="podcast_kokoro.wav", duration_minutes=5):
        ...
        # Generate dialogue with Claude via Bedrock
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate conversational podcasts with Kokoro-82M TTS (EC2 optimized)",
...
Note: This version is optimized for EC2 with IAM roles.
Ensure your instance has bedrock:InvokeModel permission.
        """
    )
    ...
    parser.add_argument("--cpu", 
                       action="store_true",
                       help="Force CPU usage (GPU used by default if available)")
