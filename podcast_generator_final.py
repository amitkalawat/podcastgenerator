...
Version 5.1 - With Speed Control for Natural Sounding Speech
"""
...
class PodcastGenerator:
    ...
    def extract_content_from_url(self, url):
        ...
        try:
            ...
            return title, text[:10000]  # Limit content length
        except Exception as e:
            print(f"Error extracting content: {e}")
            return "Content", "Failed to extract content from URL."
    
    def generate_dialogue_with_bedrock(self, content, title):
...
Dialogue Guidelines:
- Generate 10-15 short conversational exchanges
...
Content Focus:
- Cover the main topic and 3-5 key points
...
[S2] Thanks for breaking this down for our listeners!"""
    ...
    def generate_audio_with_dia(self, dialogue_text, output_file):
        """Generate audio using Dia model with optional voice cloning and speed control"""
        ...
        try:
            ...
            if self.voice_clone_audio and self.voice_clone_transcript:
                # Voice cloning: prepend the clone transcript to the dialogue
                print("Generating with voice cloning...")
                full_text = self.voice_clone_transcript.strip() + "\n" + dialogue_text
                
                # Generate with voice cloning parameters
                # Adjusted for more natural speech:
                # - Lower temperature (1.3-1.5) for more controlled output
                # - Slightly lower cfg_scale (3.5) for more natural flow
                output = self.tts_model.generate(
                    full_text,
                    audio_prompt=self.voice_clone_audio,
                    use_torch_compile=False,
                    verbose=True,
                    cfg_scale=3.5,      # Slightly lower for more natural flow
                    temperature=1.4,    # Lower temperature for controlled output
                    top_p=0.92,        # Slightly higher for more variety
                ...
                # Adjusted parameters for natural speech
                output = self.tts_model.generate(
                    dialogue_text,
                    use_torch_compile=False,
                    verbose=True,
                    cfg_scale=2.8,      # Lower for more natural speech
    ...
    def create_podcast(self, content_source, output_file="podcast.wav"):
        ...
        dialogue = self.generate_dialogue_with_bedrock(content, title)
        
        # Generate audio with Dia
        print("\n3. Generating audio with Dia-1.6B...")
        if self.voice_clone_audio:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Standard generation with slower speech
  python %(prog)s --url https://example.com/article --speed 0.85
  
  # Voice cloning with custom speed
  python %(prog)s --url https://example.com/article \\
    ...
    # Generate podcast
    generator.create_podcast(args.url, args.output)
