...
Version 5.8 - Fixed Trimming with Silence Detection
"""
...
class PodcastGenerator:
    ...
    def find_silence_boundaries(self, audio_data, sample_rate, window_ms=50, silence_threshold=0.01):
        ...
        # Find silence regions (RMS below threshold)
        silence_regions = []
        in_silence = False
        start_idx = 0
        ...
        for i, rms in enumerate(rms_values):
            if rms < silence_threshold:
                ...
                if in_silence:
                    ...
                    silence_regions.append((start_sample, end_sample, duration_ms))
                    in_silence = False
        
        return silence_regions
    
    def find_trim_point_by_silence(self, audio_data, sample_rate, min_silence_duration_ms=200, min_time_after_start=2.0):
        """Find the best trim point based on silence detection"""
        silence_regions = self.find_silence_boundaries(audio_data, sample_rate)
        
        # Look for significant silence after at least min_time_after_start seconds
        min_sample = int(min_time_after_start * sample_rate)
        
        for start, end, duration in silence_regions:
            if start > min_sample and duration > min_silence_duration_ms:
                # Found a good silence region, trim at the end of it
                print(f"  Found silence at {start/sample_rate:.2f}s, duration: {duration:.0f}ms")
                return end
...
[S2] Thanks for breaking this down for our listeners!"""
    ...
    def generate_audio_chunk(self, dialogue_chunk, chunk_index, total_chunks):
        ...
        try:
            ...
            if needs_trimming and chunk_index == 0:
                ...
                print("  Attempting silence-based trimming...")
                trim_point = self.find_trim_point_by_silence(audio_data, sample_rate, min_time_after_start=2.0)
                
                if trim_point is not None:
                    audio_data = audio_data[trim_point:]
                    print(f"  Trimmed using silence detection at {trim_point/sample_rate:.2f}s")
                else:
                    # Fallback to character-based estimation with safety buffer
                    print("  No suitable silence found, using conservative character-based trimming")
                    trim_ratio = len(trim_text) / len(full_text)
                    # More conservative: use 95% of estimate and add safety buffer
                    trim_samples = int(len(audio_data) * trim_ratio * 0.95)
                    # Add 100ms safety buffer
                    buffer_samples = int(0.1 * sample_rate)
                    trim_samples = max(0, trim_samples - buffer_samples)
                    audio_data = audio_data[trim_samples:]
                    print(f"  Trimmed {trim_samples/sample_rate:.2f}s (conservative estimate with buffer)")
            
            # Apply speed adjustment
            if self.speech_speed != 1.0:
                audio_data = self.adjust_audio_speed(audio_data, sample_rate, self.speech_speed)
            
            # Log final chunk duration
            final_duration = len(audio_data) / sample_rate
            print(f"  Final chunk duration: {final_duration:.1f} seconds")
            ...
            with open(debug_text, 'w', encoding='utf-8') as f:
                ...
                if needs_trimming:
                    f.write(f"\n\nNote: Voice clone transcript was prepended and trimmed")
                    f.write(f"\nOriginal duration: {original_duration:.1f}s")
                    f.write(f"\nFinal duration: {final_duration:.1f}s")
    ...
    def create_podcast(self, content_source, output_file="podcast.wav", duration_minutes=5):
        ...
        print("\n" + "="*50)
        print("Creating podcast (fixed trimming strategy)...")
        print("="*50 + "\n")
        ...
        print("\n3. Generating audio with Dia-1.6B (fixed trimming)...")
        if self.voice_clone_audio:
            print(f"   Using voice clone from: {self.voice_clone_audio}")
        print(f"   Speech speed: {self.speech_speed}x")
        print(f"   Strategy: Smart chunking with improved trimming")
        print(f"   Trimming: Silence detection with conservative fallback")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate conversational podcasts with fixed trimming",
