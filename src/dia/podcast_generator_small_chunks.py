...
Version 5.5 - Small Chunk Strategy for Complete Audio Coverage
"""
...
class PodcastGenerator:
    def __init__(self, use_gpu=True, aws_region='us-west-2', seed=None, voice_clone_audio=None, voice_clone_transcript=None, speech_speed=0.85):
        ...
        self.voice_clone_established = False
...
[S2] Thanks for breaking this down for our listeners!"""
    ...
    def establish_voice_clone(self):
        """Establish voice cloning by processing the transcript once"""
        if not self.voice_clone_audio or not self.voice_clone_transcript or self.voice_clone_established:
            return
        
        try:
            print("\nEstablishing voice clone characteristics...")
            ...
            self.voice_clone_established = True
            print("✓ Voice clone characteristics established")
        except Exception as e:
            print(f"Warning: Failed to establish voice clone: {e}")
    ...
    def generate_audio_chunk(self, dialogue_chunk, chunk_index, total_chunks):
        ...
        try:
            ...
            print(f"  Text preview: {dialogue_chunk[:100]}...")
            print(f"  Text length: {len(dialogue_chunk)} characters")
    ...
    def validate_chunks(self, dialogue_lines, chunks):
        """Validate that all dialogue lines are included in chunks"""
        # Reconstruct dialogue from chunks
        reconstructed_lines = []
        for chunk in chunks:
            reconstructed_lines.extend(chunk.split('\n'))
        
        # Compare counts
        original_count = len(dialogue_lines)
        reconstructed_count = len(reconstructed_lines)
        
        print(f"\nValidation:")
        print(f"  Original dialogue lines: {original_count}")
        print(f"  Lines in chunks: {reconstructed_count}")
        
        if original_count != reconstructed_count:
            print("  ⚠️  WARNING: Line count mismatch!")
            # Find missing lines
            for i, line in enumerate(dialogue_lines):
                if i >= len(reconstructed_lines) or line != reconstructed_lines[i]:
                    print(f"    Missing or different at line {i+1}: {line[:50]}...")
        else:
    ...
    def concatenate_audio_chunks(self, audio_chunks, sample_rate, output_file):
        ...
        # Add small silence between chunks for better flow (0.3 seconds)
    ...
    def generate_audio_with_dia(self, dialogue_text, output_file):
        """Generate audio using Dia model with very small chunks"""
        if not DIA_AVAILABLE or not self.tts_model:
            print("⚠️  Dia model not available, skipping audio generation")
            return None
        
        try:
            # Establish voice clone if needed
            if self.voice_clone_audio:
                self.establish_voice_clone()
            ...
            # Validate chunks
            self.validate_chunks(dialogue_lines, chunks)
    ...
    def create_podcast(self, content_source, output_file="podcast.wav", duration_minutes=5):
        ...
        print("\n3. Generating audio with Dia-1.6B (small chunk strategy)...")
        if self.voice_clone_audio:
            print(f"   Using voice clone from: {self.voice_clone_audio}")
        print(f"   Speech speed: {self.speech_speed}x")
        print(f"   Chunk size: 4 lines (~29 seconds per chunk)")
        ...
        if audio_file:
            ...
            with open("audio_generation_report.txt", 'w') as f:
                f.write("Audio Generation Report\n")
                f.write("="*50 + "\n")
                f.write(f"Output file: {output_file}\n")
                f.write(f"Dialogue file: generated_dialogue.txt\n")
                f.write(f"Debug chunks: debug_chunk_*_audio.wav\n")
                f.write("\nTo verify completeness:\n")
                f.write("1. Check each debug_chunk_*_text.txt matches its audio\n")
                f.write("2. Compare with generated_dialogue.txt\n")
                f.write("3. Listen to the complete output file\n")
            print("  ✓ Audio generation report saved")
        else:
