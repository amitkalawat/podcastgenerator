...
Version 5.3 - Chunked Generation for Longer Podcasts
"""
...
[S2] Thanks for breaking this down for our listeners!"""
    ...
    def split_dialogue_into_chunks(self, dialogue_text, chunk_size=15):
    ...
    def generate_audio_chunk(self, dialogue_chunk, chunk_index):
        ...
        try:
            # For voice cloning, prepend transcript only to first chunk
            if self.voice_clone_audio and self.voice_clone_transcript and chunk_index == 0:
                print(f"Generating chunk {chunk_index + 1} with voice cloning...")
                ...
                # For subsequent chunks, still use voice cloning if available
                print(f"Generating chunk {chunk_index + 1}...")
                if self.voice_clone_audio:
                    # Continue using voice cloning for consistency
                    output = self.tts_model.generate(
                        dialogue_chunk,
                        audio_prompt=self.voice_clone_audio,
                        use_torch_compile=False,
    ...
    def generate_audio_with_dia(self, dialogue_text, output_file):
        ...
        try:
            ...
            chunks = self.split_dialogue_into_chunks(dialogue_text, chunk_size=15)
            ...
            for i, chunk in enumerate(chunks):
                audio_data, sr = self.generate_audio_chunk(chunk, i)
